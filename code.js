...
function hexToRgba(hex) { return Object.assign(Object.assign({}, hexToRgb(hex)), { a: 1 }); }
function computeGradientTransform(from, to, bounds) {
...
figma.ui.onmessage = (msg) => {
    ...
    function createRay(pA, pB, pC, pD, from, to, idx, count) {
    ...
    for (let i = 0; i < wallRays; i++) {
        ...
        // Левая стена
        createRay({ x: 0, y: yO1 }, { x: topLeft.x, y: yI1 }, { x: topLeft.x, y: yI2 }, { x: 0, y: yO2 }, { x: 0, y: (yI1 + yI2) / 2 }, { x: topLeft.x, y: (yI1 + yI2) / 2 }, i, wallRays);
        // Правая стена
        createRay({ x: width, y: yO1 }, { x: topRight.x, y: yI1 }, { x: topRight.x, y: yI2 }, { x: width, y: yO2 }, { x: width, y: (yI1 + yI2) / 2 }, { x: topRight.x, y: (yI1 + yI2) / 2 }, i, wallRays);
    }
    ...
    for (let i = 0; i < floorRays; i++) {
        ...
        // для пола инвертируем направление: from = pD (пол), to = pC (окно)
        createRay({ x: xO1, y: height }, // pA
        { x: xI1, y: bottomLeft.y }, // pB
        { x: xI2, y: bottomLeft.y }, // pC (конец для градиента)
        { x: xO2, y: height }, // pD (начало для градиента)
        ...
        const xI1 = bottomLeft.x + (i * iw) / floorRays;
        const xI2 = bottomLeft.x + ((i + 1) * iw) / floorRays;
        createRay({ x: xO1, y: height }, // pA (внешний левый)
        { x: xI1, y: bottomLeft.y }, // pB (внутренний левый)
        { x: xI2, y: bottomLeft.y }, // pC (внутренний правый)
        { x: xO2, y: height }, // pD (внешний правый)
        { x: xI2, y: bottomLeft.y }, // from: точка пересечения B и D на уровне окна (начало)
        { x: xI2, y: height }, // to: точка пересечения G и D на уровне пола (конец)
        i, floorRays);
        // Потолок (градиент снизу-вверх)
        const xJ1 = topLeft.x + (i * iw) / floorRays;
        const xJ2 = topLeft.x + ((i + 1) * iw) / floorRays;
        createRay({ x: xJ1, y: topLeft.y }, // pA
        { x: xO1, y: 0 }, // pB
        { x: xO2, y: 0 }, // pC (start)
